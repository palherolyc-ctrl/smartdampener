...
/* Config LL link buffer */
/* Note1: Per connection has 3 fixed link buffer; All connections share the configurable link buffer( LL_LINKBUF_CFG_NUM ). */

/* Note2: When LL_LINKBUF_CFG_NUM < (BLE_MAX_ALLOW_PKT_PER_EVENT_TX + BLE_MAX_ALLOW_PKT_PER_EVENT_RX - 2) * BLE_MAX_ALLOW_CONNECTION, 
        some connection may not send/receive the expected number of packets. */
...
static void hal_low_power_io_init(void)
{
    ...
    ioinit_cfg_t ioInit[]=
    {
        ...
        {GPIO_P07,   GPIO_PULL_UP    },  // I2C SCL - needs pull-up for I2C protocol
        ...
        {GPIO_P31,   GPIO_PULL_UP    },  // I2C SDA - needs pull-up for I2C protocol
...
int  main(void)
{
    mpu9250_interface_debug_print("Debug print test from main!\n");		// 测试调试打印
